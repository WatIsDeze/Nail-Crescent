...
void SP_func_water(edict_t* self)
{
    ...
    Vec3_Copy_(self->s.origin, self->pos1);
    ...
    Vec3_MA_(self->pos1, self->moveinfo.distance, self->movedir, self->pos2);

    // if it starts open, switch the positions
    if (self->spawnflags & DOOR_START_OPEN) {
        Vec3_Copy_(self->pos2, self->s.origin);
        Vec3_Copy_(self->pos1, self->pos2);
        Vec3_Copy_(self->s.origin, self->pos1);
    }

    Vec3_Copy_(self->pos1, self->moveinfo.start_origin);
    Vec3_Copy_(self->s.angles, self->moveinfo.start_angles);
    Vec3_Copy_(self->pos2, self->moveinfo.end_origin);
    Vec3_Copy_(self->s.angles, self->moveinfo.end_angles);
