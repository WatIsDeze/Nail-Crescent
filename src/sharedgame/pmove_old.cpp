...
static void PM_StepDown(trace_t* trace) {
    ...
    Vec3_Copy_(pm->s.origin, trace->endpos);
...
static qboolean PM_TestPosition(void)
{
    ...
    Vec3_Copy_(pm->s.origin, origin);
    Vec3_Copy_(pm->s.origin, end);
...
static void PM_FinalizePosition(qboolean testForValid) {
    ...
    Vec3_Copy_(pml.velocity, pm->s.velocity);
    Vec3_Copy_(pml.origin, pm->s.origin);
    ...
    Vec3_Copy_(pml.previous_origin, pm->s.origin);
}
...
static void PM_TestInitialPosition(void)
{
    ...
    if (PM_TestPosition()) {
        ...
        Vec3_Copy_(pm->s.origin, pml.origin);
        Vec3_Copy_(pm->s.origin, pml.previous_origin);
        return;
    }
}
...
void PMove(pm_move_t* pmove, pmoveParams_t* params)
{
    ...
    Vec3_Copy_(pm->s.origin, pml.origin);
    Vec3_Copy_(pm->s.velocity, pml.velocity);
    Vec3_Copy_(pm->s.origin, pml.previous_origin);  // Save old origin, just in case we get stuck.
