...
void CL_UpdateListenerOrigin(void) {
    listener_origin  = cl.refdef.vieworg;   // MATHLIB: Vec3_Copy_(cl.refdef.vieworg, listener_origin);
    listener_forward = cl.v_forward;        // MATHLIB: Vec3_Copy_(cl.v_forward, listener_forward);
    listener_right   = cl.v_right;          // MATHLIB: Vec3_Copy_(cl.v_right, listener_right);
    listener_up      = cl.v_up;             // MATHLIB: Vec3_Copy_(cl.v_up, listener_up);
}
...
static size_t CL_Ups_m(char *buffer, size_t size)
{
    ...
    if (!cls.demo.playback && cl.frame.clientNum == cl.clientNum &&
        ...
        // N&C: FF Precision.
        Vec3_Copy_(cl.predicted_velocity, vel);
       // Vec3_Scale_(cl.frame.ps.pmove.velocity, 0.125f, vel);
    }
