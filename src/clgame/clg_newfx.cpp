...
void CLG_Heatbeam(vec3_t start, vec3_t forward)
{
    ...
    Vec3_MA_(start, 4096, forward, end);
    ...
    Vec3_MA_(move, start_pt, vec, move);

    Vec3_Scale_(vec, step, vec);
    ...
    for (i = start_pt; i < len; i += step) {
        ...
        for (rot = 0; rot < M_PI * 2; rot += rstep) {
            ...
            if (i < 10) {
                Vec3_Scale_(cl->v_right, c * (i / 10.0), dir);
                Vec3_MA_(dir, s * (i / 10.0), cl->v_up, dir);
            }
            else {
                Vec3_Scale_(cl->v_right, c, dir);
                Vec3_MA_(dir, s, cl->v_up, dir);
            }
...
void CLG_ParticleSteamEffect(vec3_t org, vec3_t dir, int color, int count, int magnitude)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        Vec3_Scale_(dir, magnitude, p->vel);
        d = crand() * magnitude / 3;
        Vec3_MA_(p->vel, d, r, p->vel);
        d = crand() * magnitude / 3;
        Vec3_MA_(p->vel, d, u, p->vel);
...
void CLG_ParticleSteamEffect2(cl_sustain_t* self)
{
    ...
    for (i = 0; i < self->count; i++) {
        ...
        Vec3_Scale_(dir, self->magnitude, p->vel);
...
void CLG_TrackerTrail(vec3_t start, vec3_t end, int particleColor)
{
    ...
    Vec3_Copy_(vec, forward);
    vectoangles2(forward, angle_dir);
    AngleVectors(angle_dir, forward, right, up);

    dec = 3;
    Vec3_Scale_(vec, 3, vec);
    ...
    while (len > 0) {
        ...
        dist = Vec3_Dot(move, forward);
        Vec3_MA_(move, 8 * cos(dist), up, p->org);
        for (j = 0; j < 3; j++) {
            p->vel[j] = 0;
            p->accel[j] = 0;
        }
...
void CLG_Widowbeamout(cl_sustain_t* self)
{
    ...
    for (i = 0; i < 300; i++) {
        ...
        Vec3_MA_(self->org, (45.0 * ratio), dir, p->org);
    }
}
...
void CLG_Nukeblast(cl_sustain_t* self)
{
    ...
    for (i = 0; i < 700; i++) {
        ...
        Vec3_MA_(self->org, (200.0 * ratio), dir, p->org);
    }
}
...
void CLG_WidowSplash(void)
{
    ...
    for (i = 0; i < 256; i++) {
        ...
        dir[2] = crand();
        VectorNormalize(dir);
        Vec3_MA_(teParameters.pos1, 45.0, dir, p->org);
        Vec3_MA_(vec3_origin, 40.0, dir, p->vel);
...
void CLG_Tracker_Explode(vec3_t  origin)
{
    ...
    for (i = 0; i < 300; i++) {
        ...
        dir[2] = crand();
        VectorNormalize(dir);
        Vec3_Scale_(dir, -1, backdir);

        Vec3_MA_(origin, 64, dir, p->org);
        Vec3_Scale_(backdir, 64, p->vel);
    }
}
...
void CLG_TrapParticles(entity_t* ent)
{
    ...
    ent->origin[2] -= 14;
    Vec3_Copy_(ent->origin, start);
    Vec3_Copy_(ent->origin, end);
    end[2] += 64;
