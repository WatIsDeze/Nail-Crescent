...
vkpt_evaluate_sun_light(sun_light_t* light, const vec3_t sky_matrix[3], float time)
{
	...
	Vec3_Copy_(sun_direction_world, light->direction);
}
...
vkpt_physical_sky_update_ubo(QVKUniformBuffer_t * ubo, const sun_light_t* light, qboolean render_world)
{
	...
	Vec3_Copy_(light->color, ubo->sun_color);
	Vec3_Copy_(light->direction_envmap, ubo->sun_direction_envmap);
	Vec3_Copy_(light->direction, ubo->sun_direction);

    if (light->direction[2] >= 0.99f)
    {
        Vec3_Set_(ubo->sun_tangent, 1.f, 0.f, 0.f);
        Vec3_Set_(ubo->sun_bitangent, 0.f, 1.f, 0.f);
    }
    else
    {
        vec3_t up;
