...
static int dummy_create(void)
{
    ...
    newcl->netchan = (netchan_t*)SV_Mallocz(sizeof(netchan_t)); // CPP: Cast
...
static void emit_gamestate(void)
{
    ...
    for (i = 0, ps = mvd.players; i < sv_maxclients->integer; i++, ps++) {
        ...
        MSG_WriteDeltaPlayerstate_Packet(NULL, ps, i, (msgPsFlags_t)(flags | extra));
    }
    ...
    for (i = 1, es = mvd.entities + 1; i < ge->num_edicts; i++, es++) {
        ...
        es->number = i;
        MSG_WriteDeltaEntity(NULL, es, (msgEsFlags_t)flags); // CPP: Cast
        es->number = j;
    }
    MSG_WriteShort(0);
}
...
static void copy_entity_state(entity_packed_t *dst, const entity_packed_t *src, int flags)
{
    if (!(flags & MSG_ES_FIRSTPERSON)) {
        dst->origin = src->origin; // MATHLIB: Vec3_Copy_(src->origin, dst->origin);
        dst->angles = src->angles; // MATHLIB: Vec3_Copy_(src->angles, dst->angles);
        dst->old_origin = src->old_origin; // MATHLIB: Vec3_Copy_(src->old_origin, dst->old_origin);
    }
...
static void emit_frame(void)
{
    ...
    for (i = 0; i < sv_maxclients->integer; i++) {
        ...
        if (PPS_INUSE(oldps)) {
            ...
            MSG_WriteDeltaPlayerstate_Packet(oldps, &newps, i, (msgPsFlags_t)flags); // CPP: Cast
        } else {
            // this is a new player, send it from the last state
            MSG_WriteDeltaPlayerstate_Packet(oldps, &newps, i,
                                             (msgPsFlags_t)(flags | MSG_PS_FORCE)); // CPP: Cast
        }
...
void SV_MvdMulticast(int leafnum, multicast_t to)
{
    ...
    op = (mvd_ops_t)(mvd_multicast_all + to); // CPP: Cast
...
static void flush_stream(gtv_client_t *client, int flush)
{
    ...
    do {
        data = (byte*)FIFO_Reserve(fifo, &len); // CPP: Cast
        if (!len) {
...
static void write_stream(gtv_client_t *client, void *data, size_t len)
{
    ...
    if (client->z.state) {
        ...
        do {
            ...
            z->next_out = (Bytef*)data; // CPP: Cast
            z->avail_out = (uInt)len;
...
static void parse_hello(gtv_client_t *client)
{
    ...
    data = (byte*)SV_Malloc(size); // CPP: Cast
...
void SV_MvdInit(void)
{
    ...
    mvd.players = (player_packed_t*)Z_ReservedAlloc(sizeof(player_packed_t) * sv_maxclients->integer); // CPP: Cast
    mvd.entities = (entity_packed_t*)Z_ReservedAlloc(sizeof(entity_packed_t) * MAX_EDICTS); // CPP: Cast
